-module(balanceador_props).

-include_lib("proper/include/proper.hrl").
-compile(export_all).

%% Módulo de implementación que imos probar
-define(TEST_MODULE, balanceador).
...
prop_bal(Bal) ->
    ?FORALL({_}, {nodos()},
            begin
                ...
                %ok = ?TEST_MODULE:start(Nodos),
                N = rand:uniform(2),
                Aux = lists:nth(N, [active, inactive]),
                {balanceador, Bal} ! {Aux},
                {balanceador, Bal} ! {peticion, self()},
